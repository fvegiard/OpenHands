...
```json
{
  ...
  "sha256": "64 lowercase hex characters or null; optional unless requested or needed for integrity",
  "context_checked": true,
  "notes": "Why this supports, contradicts, or fails to establish the claim"
}
```

Reject an evidence item as proof when `context_checked` is false, the file cannot be opened, or the source path is absent. A missing optional hash is not a proof failure. The item may remain as an explicitly labeled lead.
