...
class WorktreeSandboxService:
    ...
    def create_worktree(
        self, branch_name: str, target_dir: Optional[str] = None
    ) -> str:
        """
        ...
        branches = self._run_git_command(
            ['branch', '--list', branch_name], cwd=self.base_repo_path
        )
        if branch_name not in branches:
            self._run_git_command(['branch', branch_name], cwd=self.base_repo_path)

        self._run_git_command(
            ['worktree', 'add', target_dir, branch_name], cwd=self.base_repo_path
        )
        return target_dir
    ...
    def remove_worktree(self, target_dir: str):
        ...
        self._run_git_command(
            ['worktree', 'remove', '--force', target_dir], cwd=self.base_repo_path
        )
